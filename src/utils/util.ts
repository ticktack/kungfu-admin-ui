...
export function getFlatArr(menuList: Menu.MenuOptions[]) {
	...
	return newMenuList.reduce((pre: Menu.MenuOptions[], current: Menu.MenuOptions) => {
		...
		if (current.children && current.children.length) flatArr = [...flatArr, ...getFlatArr(current.children)]
		return flatArr
	}, [])
}
...
/**
 * @description 处理 返回的的用户菜单树
 * @param {Array} list 菜单树
 * @return
 * */
export function formatterMenuTree(list: any) {
	list.forEach((menu: any) => {
		menu.path = menu.component ? menu.component : '/' + menu.menuCode
		menu.name = menu.menuCode.slice(0, 1).toUpperCase() + menu.menuCode.slice(1)
		menu.meta = {
			title: menu.menuName,
			icon: menu.icon,
			isLink: '',
		...
		if (menu.children && menu.children) {
			menu.redirect = `${menu.path}/${menu.children[0].menuCode}`
			menu.children.forEach((cMenu: any) => {
				cMenu.component = `${menu.component || '/' + menu.menuCode}/${cMenu.menuCode}`
			})

			if (menu.component) delete menu.component
			formatterMenuTree(menu.children)
		} else {
